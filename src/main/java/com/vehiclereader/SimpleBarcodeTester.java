...
import com.vehiclereader.Barcode.*;
...
public class SimpleBarcodeTester {
    ...
    private static String aztecResult;

    static {
//        System.load("c:\\Users\\arek50\\Downloads\\opencv\\opencv\\build\\bin\\");
...
//        System.loadLibrary("opencv_ffmpeg340_64");
    ...
    public static void main(String[] args) {
        ...
        for (CharSequence result : results.keySet()) {
            ...
            if (showImages) {
                ...
                for (int j = 0; j < 3; j++)
                    Imgproc.line(image, rectPoints[j], rectPoints[j + 1], colour, 2, Core.LINE_AA, 0);
                Imgproc.line(image, rectPoints[3], rectPoints[0], colour, 2, Core.LINE_AA, 0);
    ...
    private static Map<CharSequence, BarcodeLocation> processCamera(String caption) {
        ...
        try {
            ...
            while (System.currentTimeMillis() < end_time) {
                ...
                if (barcode == null)
                    barcode = new MatrixBarcode(imgName, image, TryHarderFlags.ALL);
                else{
                    if(!Barcode.updateImage(barcode, image, imgName))
                        barcode = new MatrixBarcode(imgName, image, TryHarderFlags.ALL);
                }
    ...
    private static void decodeBarcode(List<CandidateResult> candidateCodes, String filename, String caption) {
        ...
        for (CandidateResult cr : candidateCodes) {
            ...
            imageBarcodeReader.setAztecFile(candidate);
            try {
