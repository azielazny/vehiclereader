 ...
 */
package com.vehiclereader.Barcode;
...
public class MatrixBarcode extends Barcode {
    ...
    // tiles must be square
        ...
        for(int i = 0, row_offset = 0; i < rows; i += tileSize, row_offset += probMatTileSize){
            ...
            bottom_row = Math.min((i + tileSize), rows);
            prob_mat_bottom_row = Math.min((row_offset + probMatTileSize), img_details.probMatRows);
            ...
            for(int j = 0, col_offset = 0; j < cols; j += tileSize, col_offset += probMatTileSize){
                ...
                right_col = Math.min((j + tileSize), cols);
                prob_mat_right_col = Math.min((col_offset + probMatTileSize), img_details.probMatCols);
